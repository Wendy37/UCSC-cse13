...
int uv_census(Universe *u, int r, int c){
    ...
    if (!uv_toroidal(u)){  // flat universe
        ...
        for (int i = c-1; i < c+2; i++){
            ...
            else if (!uv_inbound(u, r-1, i) && uv_get_cell(u, new_row(uv_rows(u), r-1), new_col(uv_cols(u), i))){
                    live++;
            }
        }
        for (int i = c-1; i < c+2; i += 2){
            ...
            else if (!uv_inbound(u, r, i) && uv_get_cell(u, new_row(uv_rows(u), r), new_col(uv_cols(u), i))){
                    live++;
            }
        }
        for (int i = c-1; i < c+2; i++){
