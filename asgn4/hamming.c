...
void int_to_mat(uint8_t integer,
                BitMatrix *m) { // convert integer to a bit matrix
  uint8_t mask;
  uint8_t bit;
...
void mat_mult(BitMatrix *a, BitMatrix *b,
              BitMatrix *c) { // matrices multiplication a = b * c
  uint8_t bit;
